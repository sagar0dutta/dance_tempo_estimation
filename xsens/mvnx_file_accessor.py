...
class MvnxFileAccessor:
    ...
    def get_segment_pos(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        Get the position information for a segment

        :param segment: The index of the segment to return the data for (xsens.mvn.SEGMENT_.... )
        :param frame: Can be the index of the frame to return, a slice to return a range of frames
                      or xsens.mvn.FRAMES_ALL (default) to return all frames
        :param axis: The frame number to return the data for (xsens.mvn.AXIS_... ALL for all axes)
        :return: A single value, list, or list of lists with position values
        """

        return self.get_segment_data('pos', segment, frame, axis)

    def get_segment_ori(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
    ...
    def get_point_pos(self, segment, point):
        ...
        if segment_name == 'LeftFoot':
            point_name = xsens.mvn.POINTS_LEFT_FOOT[point]
        elif segment_name == 'RightFoot':
            point_name = xsens.mvn.POINTS_RIGHT_FOOT[point]
        elif segment_name == 'LeftToe':
            point_name = xsens.mvn.POINTS_LEFT_TOE[point]
        elif segment_name == 'RightToe':
            point_name = xsens.mvn.POINTS_RIGHT_TOE[point]

        return self.file_data['segments'][segment_name]['points_mvn'][point_name]

    def get_segment_vel(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
    ...
    def get_segment_acc(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
    ...
    def get_segment_angular_vel(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        Get the acceleration information for a segment
    ...
    def get_segment_angular_acc(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        Get the acceleration information for a segment
    ...
    def get_segment_data(self, data_field, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        return self.get_data('segment_data', data_field, segment, frame, axis)

    """ Joint methods """

    def get_joint_angle(self, joint, frame=xsens.mvn.FRAMES_ALL, angle=xsens.mvn.ANGLE_ALL):
        joint_name = xsens.mvn.JOINTS[joint]
        data_set = 'joint_data'

        frame, is_single_frame = self.frame_to_mapped_slice(frame)

        if angle == xsens.mvn.ANGLE_ALL:
    ...
    def get_joint_angle_xzy(self, joint, frame=xsens.mvn.FRAMES_ALL, angle=xsens.mvn.ANGLE_ALL):
        joint_name = xsens.mvn.JOINTS[joint]
    ...
    def get_ergo_joint_angle(self, joint, frame=xsens.mvn.FRAMES_ALL, angle=xsens.mvn.ANGLE_ALL):
        joint_name = xsens.mvn.ERGO_JOINTS[joint]
    ...
    def get_center_of_mass_pos(self, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        Get the position information for center of mass

        :param frame: Can be the index of the frame to return, a slice to return a range of frames
                      or xsens.mvn.FRAMES_ALL (default) to return all frames
        :param axis: The axis to return the data for (xsens.mvn.AXIS_... ALL for all axes)
        :return: A single value, list, or list of lists with position values
        """

        return self.get_segment_data('pos', xsens.mvn.SEGMENT_CENTER_OF_MASS, frame, axis)

    def get_center_of_mass_vel(self, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        ...
        return self.get_segment_data('vel', xsens.mvn.SEGMENT_CENTER_OF_MASS, frame, axis)

    def get_center_of_mass_acc(self, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        ...
        return self.get_segment_data('acc', xsens.mvn.SEGMENT_CENTER_OF_MASS, frame, axis)

    """ Sensor methods """

    def get_sensor_ori(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
        Get the orientation information for a sensor

        :param segment: The index of the segment to return the sensor data for (xsens.mvn.SEGMENT_.... )
        :param frame: Can be the index of the frame to return, a slice to return a range of frames
                      or xsens.mvn.FRAMES_ALL (default) to return all frames
        :param axis: The frame number to return the data for (xsens.mvn.AXIS_... ALL for all axes)
        :return: A single value, list, or list of lists with orientation values
    ...
    def get_sensor_free_acc(self, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        """
    ...
    def get_sensor_data(self, data_field, sensor_segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
    ...
    def get_foot_contacts(self, frame):
        ...
        """
        frame, is_single_frame = self.frame_to_mapped_slice(frame)
        return_values = self.file_data['frames']['contacts_data'][frame]
        return return_values[0] if is_single_frame else return_values
    ...
    def has_foot_contact(self, frame, foot_contact_flags=0):
        ...
        :param foot_contact_flags: The specific contact to check for:
                                    xsens.mvn.FOOT_CONTACT_LEFT_HEEL,
                                    xsens.mvn.FOOT_CONTACT_LEFT_TOE,
                                    xsens.mvn.FOOT_CONTACT_RIGHT_HEEL or
                                    xsens.mvn.FOOT_CONTACT_RIGHT_TOE

                                 It is possible to combine contacts by summing the values, eg:
                                    xsens.mvn.FOOT_CONTACT_LEFT_HEEL + xsens.mvn.FOOT_CONTACT_LEFT_TOE
    ...
    def get_data(self, data_set, data_field, segment, frame=xsens.mvn.FRAMES_ALL, axis=xsens.mvn.AXIS_ALL):
        if segment == -1:
            segment_name = 'com'
        else:
            segment_name = xsens.mvn.SEGMENTS[segment]
